...
$query1="SELECT clients.c_name, branch.b_name, users.fname,users.lname, assigned.assigndate,clients.edate FROM clients, users, branch,assigned WHERE clients.c_id= assigned.cid AND branch.b_id=assigned.b_id AND users.id=assigned.uid AND assigned.cid=$client AND assigned.assigndate LIKE '$year-$month-%'";
$select_all_assigned= $mysqli->query($query1);
?>
